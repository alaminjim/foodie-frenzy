...
import { socialIcons } from "../../../public/FOODIE-FRENZY-FINAL/dummydata";
...
const Footer = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    toast.success(`Thank's for subscribing! We'll send updates to ${email}`);
    setEmail("");
  };
  return (
    <footer className="bg-[#2A211C] text-amber-100 py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 mb-12">
          ...
          <div className="space-y-6">
            ...
            <p className="text-amber-200/90 text-sm font-sacramento italic">
              When culinary artistry meets doorstep convenience.<br></br>
              Savor handcrafted perfection, delivered with care.
            </p>
            <form onSubmit={handleSubmit} className="relative mt-4 group">
              <div className="flex items-center gap-2 mb-2">
                <FaRegEnvelope className="text-amber-400 animate-pulse"></FaRegEnvelope>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full px-4 py-2.5 rounded-lg bg-amber-50/5 border-2 border-amber-400/30 focus:outline-none focus:border-amber-400 focus:ring-4 focus:ring-amber-400/20 transition-all duration-300 placeholder-amber-200"
                  required
                />
                <button
                  type="submit"
                ...
                >
                  ...
                  <BiChevronRight className="text-xl transition-transform duration-300 group-hover:animate-spin flex-shrink-0"></BiChevronRight>
              ...
              <ul className="space-y-3">
                {navItems.map((items, i) => (
                  <li key={i}>
                    ...
                    >
                      <BiChevronRight className="mr-2 text-amber-400 group-hover:animate-bounce"></BiChevronRight>
                      <span className="hover:italic">{items.name}</span>
                    </a>
                  </li>
                ))}
              </ul>
              ...
              <div className="flex space-x-4">
                {socialIcons.map(({ icon: Icon, link, color, label }, idx) => (
                  <a
                    ...
                    target="_blank"
                    href={link}
                    className="text-2xl bg-amber-400/10 rounded-full hover:bg-amber-400/20 hover:scale-110 transition-all duration-300 relative group p-2"
                    style={{ color }}
                  >
                    <Icon />
        ...
        <div className="border-t border-amber-800 pt-8 flex flex-col items-center">
          <p className="w-full text-amber-400 text-lg mb-2 font-playfair text-center">
